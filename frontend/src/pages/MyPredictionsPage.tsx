    ...
    <section>
      ...
      <div className="space-y-3">
        {predictions.map((prediction) => (
          <article key={prediction.id} className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm">
            ...
            <div className="mt-3 flex items-center justify-between rounded-lg bg-slate-50 px-3 py-2 text-sm">
              ...
              <strong>{prediction.points} pts</strong>
            </div>
          </article>
        ))}
        {predictions.length === 0 ? <p className="rounded-lg bg-white p-4 text-sm text-slate-500">Voce ainda nao enviou palpites.</p> : null}
      </div>
    </section>
  );
}
